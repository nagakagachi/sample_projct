...
namespace ngl
{
	namespace memory
	{
		...
		public:
			...
			bool Initialize(void* mem, u64 size, u32 secondLevelExponentiation = 3);
		...
		public:
			// 第一レベルインデックス
			s32 GetFirstLevelIndex(u64 requireSize);
			// 第二レベル
			s32 GetSecondLevelIndex(u64 requireSize, u32 fli, u32 secondLevelExp);

			// フリーリストでFLIより大きい最小のFLIを返す
			s32 GetFreeListFirstLevelIndex(u32 fli);
			// フリーリストで指定したFLI内で最小のSLIを返す
			s32 GetFreeListSecondLevelIndex(u32 fli);
		...
		private:
			...
			BoundaryTagBlock* RemoveFreeListTop(u32 fli, u32 sli);
		...
		private:
			void* head_;
			u64 size_;
			u32 secondLevelExponentiation_;

			// 管理領域の先頭と末尾にマーカーとしてからのタグを仕込む 
			BoundaryTagBlock*	headTag_;
			BoundaryTagBlock*	tailTag_;

			u64					freeListBitFli_;// 0-63 の各第一レベルフリーリストの有無を示すビット
			u32					freeListBitSli_[64];// 対応する第一レベル内部の第二レベルフリーリストの有無を示すビット

			// 分割幅レベル以下の要素は使われないけどまあいいか　
			BoundaryTagBlock*	freeList_[64][SECOND_LEVEL_INDEX_MAX];
		};
	}
}
