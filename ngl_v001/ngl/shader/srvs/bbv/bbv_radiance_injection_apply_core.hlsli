...
#ifndef NGL_SRVS_RADIANCE_ENABLE_SHORT_RAY_FALLBACK
...
)
{
    ...
    // 追加間引き: 既存の2x2位相更新の上に、ピクセル単位でさらにサンプル密度を下げる。
        ...
        const uint pixel_skip_hash = (uint(src_texel_in_view.x) + uint(src_texel_in_view.y) * 3u + cb_srvs.frame_count) % k_bbv_radiance_injection_pixel_skip_stride;
    ...
    // 追加間引き: フレーム単位でRadiance注入自体をスキップし、更新コストを抑える。
        ...
        if(0 != (cb_srvs.frame_count % (k_bbv_radiance_injection_frame_skip_count + 1)))
        {
            return;
        }
    #endif
    ...
    if(cb_srvs.bbv_update_flow_mode == k_bbv_update_flow_depthtest)
    {
        ...
        if(to_pixel_len_sq > 1e-10)
        {
            ...
            pos_ws += ray_dir_ws * cb_srvs.bbv_depthtest_injection_world_offset;
        }
    }
